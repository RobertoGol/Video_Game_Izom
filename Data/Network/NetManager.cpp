...
bool NetManager::StartServer(unsigned short port) 
{
    ...
    if (m_UdpSocket == INVALID_SOCKET) { WSACleanup(); return false; }
...
{
    if (!m_IsActive || m_UdpSocket == INVALID_SOCKET) return;
...
void NetManager::PollIncomingPackets() 
{
    ...
    while (true) 
    {
        ...
        if (bytesRead == SOCKET_ERROR) 
        {
            ...
            if (err == WSAEWOULDBLOCK) break; // Данные в сетевой карте закончились, выходим
            break;
        }

        if (bytesRead == sizeof(PlayerStatePacket) && buffer.type == NetPacketType::PlayerState) 
        {
            if (buffer.accountID == m_LocalAccountID) continue; // Игнорируем свои же эхо-пакеты
