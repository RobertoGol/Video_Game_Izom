...
namespace bunker
{
    ...
    bool LoadEnemyProfileFromAut(const std::string &filePath, AutEnemyProfile &profileOut)
    {
        ...
        while (std::getline(file, line))
        {
            ...
            if (std::getline(ss, key, '=') && std::getline(ss, value))
            {
                ...
                if (key == "type")
                {
                    if (value == "VerminSwarm")
                        profileOut.classType = EnemyClassType::VerminSwarm;
                    if (value == "TargetDummy")
                        profileOut.classType = EnemyClassType::TargetDummy;
                    if (value == "PunchingBag")
                        profileOut.classType = EnemyClassType::PunchingBag;
                    if (value == "StaticBase")
                        profileOut.classType = EnemyClassType::StaticBase;
                }
                else if (key == "health")
                {
                    profileOut.maxHealth = std::stof(value);
                }
                else if (key == "speed")
                {
                    profileOut.baseSpeed = std::stof(value);
                }
                else if (key == "radius")
                {
                    profileOut.physicalRadius = std::stof(value);
                }
                else if (key == "erosion")
                {
                    profileOut.erosionDamage = std::stof(value);
                }
                else if (key == "score")
                {
                    profileOut.rewardScore = std::stoi(value);
                }
                else if (key == "faction")
                {
                    profileOut.factionID = value;
                }
                else if (key == "name")
                {
    ...
    void UpdateEnemiesAI(float deltaTime)
    {
        ...
        for (auto &e : enemies)
        {
            ...
            { // Радиус обнаружения игрока 7 клеток (49 в квадрате)
                targetX = playerPos.x;
                targetY = playerPos.y;
            }
