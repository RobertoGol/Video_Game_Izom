...
namespace bunker
{
    ...
    public:
        ...
        // Оставляем это объявление здесь (строго ОДИН раз)
        ScreenPoint WorldToScreen(const Vector3D &worldPos, const Vector3D &cTarget) const;
        Vector3D ScreenToWorldGround(const ScreenPoint &screenPos, const Vector3D &cTarget) const;

        // Метод камеры внутри класса
        void UpdateHelldiversCamera(float deltaTime);

        // --- ДОПOЛНИТЕЛЬНЫЙ БЛОК: РЕЖИМ FALLOUT 4 ---
    ...
    public:
        ...
        {
            currentViewMode = (currentViewMode == CameraViewMode::ClassicIsometric) ? CameraViewMode::ThirdPersonFallout : CameraViewMode::ClassicIsometric;
        }
        ...
        // ИСПРАВЛЕНО: Повторные объявления WorldToScreen и ScreenToWorldGround удалены отсюда!
    };
