        ...
        if (tx < 0 || tx >= MAP_WIDTH || ty < 0 || ty >= MAP_HEIGHT || currentSectorMap[tx][ty] == 1) 
        {
            ...
            if (tx >= 0 && tx < MAP_WIDTH && ty >= 0 && ty < MAP_HEIGHT) 
            {
                if (wallDurability[tx][ty] > 0) 
                {
                    ...
                    if (wallDurability[tx][ty] <= 0) currentSectorMap[tx][ty] = 0; // Стена пробита!
                }
            }
        }
            ...
            for (auto& e : enemies) 
            {
                ...
                if (distSq <= hitRadiusSq) 
                {
                    ...
                    if (GetAsyncKeyState(VK_F10) & 0x8000) 
                    {
                        ...
                        {
                            if (!nextTarget.isAlive) continue;
                            ...
                            if ((ldx * ldx + ldy * ldy) <= chainRadiusSq) 
                            {
                                ...
                                ScreenPoint ndcStart = PixelsToNDC(pStart.x, pStart.y, 1280.0f, 720.0f);
                                ScreenPoint ndcEnd   = PixelsToNDC(pEnd.x, pEnd.y, 1280.0f, 720.0f);
    ...
    if (enemySpawnTimer >= 3.5f) 
    {
        ...
        if (enemies.size() < 60) 
        {
            ...
            Vector3D spawnPoints[4] = {
                { 1.0f, 1.0f, 0.0f }, { 18.0f, 1.0f, 0.0f },
                { 1.0f, 18.0f, 0.0f }, { 18.0f, 18.0f, 0.0f }
            };
