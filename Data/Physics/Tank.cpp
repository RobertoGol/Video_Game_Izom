...
namespace bunker
{
    ...
    void VanguardTitan::UpdateBT7274(float deltaTime)
    {
        ...
        else
        {
            ...
            if (m_CoreChargePercentage <= 0.0f)
        ...
        if (GetAsyncKeyState('1') & 0x8000)
            TriggerMechanicalReMap(AncientLoadout::XO16_SolidKinetic);
        if (GetAsyncKeyState('2') & 0x8000)
            TriggerMechanicalReMap(AncientLoadout::Scorch_ThermiteMortar);
        if (GetAsyncKeyState('3') & 0x8000)
            TriggerMechanicalReMap(AncientLoadout::Ion_SplitLaser_Vacuum);
        ...
        if (!titan.isPiloted)
        {
            ...
            if (!enemies.empty() && fireCooldown <= 0.0f && !m_VortexActive)
            {
                for (auto &e : enemies)
                {
                    ...
                    if (eDistSq <= 400.0f)
                    { // В пределах оптического дальномера 20 клеток
        ...
        if (!CheckWorldCollision(nextX, titan.position.y, btRadius))
            titan.position.x = nextX;
        if (!CheckWorldCollision(titan.position.x, nextY, btRadius))
            titan.position.y = nextY;
        ...
        if ((GetAsyncKeyState(VK_LBUTTON) & 0x8000) && fireCooldown <= 0.0f && !m_VortexActive)
        {
            ...
            if (mLenSq > 0.01f)
            {
                ...
                {
                    b.type = BulletType::Standard;
                    b.speed = 18.0f; // Тяжелая навесная кассета термита
                    bullets.push_back(b);
                    fireCooldown = 0.65f;
                }
                ...
                {
                    b.speed = 45.0f;
                    bullets.push_back(b); // Ион сплиттер пушка
                    fireCooldown = 0.14f;
                }
            }
        }
    ...
    {
        if (m_CoreChargePercentage < 100.0f)
            return false;
