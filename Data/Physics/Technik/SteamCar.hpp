...
public:
    ...
    void UpdateCarPhysics(Vector3D& carPos, float deltaTime) 
    {
        ...
        if (GetAsyncKeyState('D') & 0x8000) { m_HullAngle += 1.5f * deltaTime; }
        ...
        if (!CheckWorldCollision(nextX, carPos.y, 0.5f)) carPos.x = nextX;
        if (!CheckWorldCollision(carPos.x, nextY, 0.5f)) carPos.y = nextY;
    }
