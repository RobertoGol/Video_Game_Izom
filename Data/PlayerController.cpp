...
namespace bunker { // Входим в пространство имен нашего игрового движка
...
void PlayerController::CaptureWin32Input(HWND hwnd, CustomPlayerInput& inputOut) {
    ...
    if (GetAsyncKeyState('W') & 0x8000) { inputOut.moveForward += 1.0f; }  // Игрок жмет W -> движение вперед
    if (GetAsyncKeyState('S') & 0x8000) { inputOut.moveForward -= 1.0f; }  // Игрок жмет S -> движение назад
    if (GetAsyncKeyState('A') & 0x8000) { inputOut.moveStrafe -= 1.0f; }   // Игрок жмет A -> стрейф влево
    if (GetAsyncKeyState('D') & 0x8000) { inputOut.moveStrafe += 1.0f; }   // Игрок жмет D -> стрейф вправо
...
void PlayerController::UpdateMovement(HWND hwnd, float deltaTime, float wWidth, float wHeight, float& playerRadius) {
    ...
    if (GetAsyncKeyState('W') & 0x8000) { targetDir.x += 1.0f; targetDir.y -= 1.0f; } // W двигает по ромбу строго вверх-вправо
    if (GetAsyncKeyState('S') & 0x8000) { targetDir.x -= 1.0f; targetDir.y += 1.0f; } // S двигает строго вниз-влево
    if (GetAsyncKeyState('A') & 0x8000) { targetDir.x -= 1.0f; targetDir.y -= 1.0f; } // A двигает строго вверх-влево
    if (GetAsyncKeyState('D') & 0x8000) { targetDir.x += 1.0f; targetDir.y += 1.0f; } // D двигает строго вниз-вправо
